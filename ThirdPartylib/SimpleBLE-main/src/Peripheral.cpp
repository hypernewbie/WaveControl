...
std::vector<BluetoothUUID> Peripheral::advertised_services() {
...
void Peripheral::write_desc(BluetoothUUID service, BluetoothUUID characteristic, uint32_t custom_value) {
    if (!is_connected()) {
        throw Exception::OperationFailed();
    }

    internal_->write_desc(service, characteristic, custom_value);
}
